...
import { BaseCacheManager, BASE_SNAPSHOT_BIN, computeHash } from "./base-cache.js";
...
export default function piKvCacheManager(pi: ExtensionAPI): void {
  ...
  const activateSessionSlot = async (
    ctx: ExtensionContext,
    systemPromptOverride?: string,
    toolsOverride?: unknown[]
  ): Promise<number> => {
    const sessionId = ctx.sessionManager?.getSessionId();
    if (!sessionId) return config.slotId;
    ...
    if (res.restored) {
      ...
      try {
        if (ctx.hasUI) {
          ...
          ctx.ui.notify(
            `Restored KV cache to slot ${res.slotId}: ${(res.tokens ?? 0).toLocaleString()} tokens in ${(res.durationMs || 0).toFixed(1)}ms (⚡ instant resume)`,
            "info"
          );
        }
      } catch {
  ...
  pi.on("before_provider_request", async (event, ctx: ExtensionContext) => {
    ...
    const systemPrompt =
      (payload?.messages && payload.messages[0]?.role === "system" ? payload.messages[0].content : undefined) ??
      (typeof ctx.getSystemPrompt === "function" ? ctx.getSystemPrompt() : undefined);
      ...
      switch (sub) {
        case "status": {
          ...
          if (sessionSnaps.length > 0) {
            ...
            for (const s of sessionSnaps) {
              ...
              lines.push(
                `| ${label} | ${s.meta.tokenCount.toLocaleString()} | ${LruManager.formatBytes(s.meta.fileSizeBytes)} | ${new Date(s.meta.lastAccessedAt).toLocaleString()} |`
              );
            }
          }
        ...
        case "save": {
          ...
          try {
            ...
            try { ctx.ui.setStatus(STATUS_KEY, `KV: ${formatTokens(tokens)} (S${slotId}) 💾`); } catch {}
            notifyUser(
              ctx,
              `✅ Successfully saved ${tokens.toLocaleString()} tokens (${LruManager.formatBytes(bytes)}) in ${saveMs.toFixed(1)}ms`,
              "info"
            );
          } catch (err) {
        ...
        case "restore": {
          ...
          try {
            ...
            notifyUser(
              ctx,
              `✅ Successfully restored ${tokens.toLocaleString()} tokens in ${restoreMs.toFixed(1)}ms`,
              "info"
            );
          } catch (err) {
            const msg = err instanceof Error ? err.message : String(err);
        ...
        case "prune": {
          try {
            ...
            if (result.prunedFiles.length > 0) {
              notifyUser(
                ctx,
                `🧹 Evicted ${result.prunedFiles.length} snapshot(s) (${LruManager.formatBytes(result.freedBytes)} freed):\n${result.prunedFiles.join(", ")}`,
                "info"
              );
            } else {
        ...
        case "base-update": {
          if (!isLlamaServerContext(ctx as unknown as ExtensionContext, config)) {
            notifyUser(
              ctx,
              `⚠️ Base cache update is only available for local llama-server models.`,
              "warning"
            );
            break;
          }
